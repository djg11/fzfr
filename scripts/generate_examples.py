#!/usr/bin/env python3
"""
generate_examples.py — Generate the fzfr examples directory.

Run from the repository root:
    python3 generate_examples.py

This creates examples/ with a small directory tree that exercises every
fzfr feature: content search, filename search, archive preview, PDF
...
def make_docs() -> None:
    write(
        ...
        fzfr uses a two-phase pipeline for content search:
        ...
        When a remote host is specified, fzfr transfers itself via SSH stdin on
        first use and caches it at `~/.cache/fzfr/`. Subsequent calls use the
        ...
        JSON file under `/dev/shm/fzfr/session-*/state.json`. Each fzf callback
...
def make_readme() -> None:
    write(
        ...
        Generated by `python3 generate_examples.py` — no binary blobs stored.
        ...
        examples/
          ...
          docs/
            architecture.md       Markdown — fzfr architecture overview
            report.pdf            PDF      — quarterly report (preview with pdftotext / rga)
          config/
            fzfr.toml             TOML     — example configuration
          data/
        ...
        fzfr local examples content

        # Filename search — fuzzy-filter by name
        fzfr local examples name
        ...
        fzfr local "examples/tricky names" name
        ```
...
if __name__ == "__main__":
    print(f"Generating examples in {ROOT}/")
    make_src()
    make_docs()
    make_config()
    make_data()
