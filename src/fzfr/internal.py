...
def _substitute_placeholders(cmd: str, path: str, paths: list[str], base: str, q: str) -> str:
    ...
    so filenames containing spaces, quotes, or semicolons cannot inject
    shell commands.

    Placeholder contract:
      {path}   — single highlighted file (shlex.quote'd)
      {paths}  — all TAB-selected files, space-joined (each shlex.quote'd);
                 falls back to {path} if no multi-select
      {dir}    — directory containing {path} (shlex.quote'd)
      {base}   — search root BASE_PATH (shlex.quote'd)
      {q}      — current fzf query string (shlex.quote'd)
    """
    import os as _os
    ...
    result = cmd
    result = result.replace("{paths}", safe_paths)
...
def cmd_internal_exec(argv: list[str]) -> int:
    """Internal: execute a custom action identified by group_key.action_key.

    Called by fzf execute-silent bindings generated by build_fzf_invocation()
    for each custom action. Looks up the action in CONFIG["custom_actions"],
    substitutes placeholders, and runs the command via the shell.

    Usage: fzfr _internal-exec <state_path> <action_id> [path ...]
      state_path  — session state file (for base_path and q)
      action_id   — "group_key.action_key" e.g. "g.a"
      path ...    — selected file paths from fzf {+} (one or more)

    Output modes:
      "tmux"    — open a new tmux window and stream command output there
      "preview" — print command stdout to our stdout (fzf reads it as preview)
      "silent"  — suppress all output; only non-zero exit is surfaced

    Security: {path} and all other placeholders are always shlex.quote()'d.
    The cmd string itself is user-controlled (same threat model as ~/.bashrc).
    """
    if len(argv) < 3:
        print(
            "Usage: fzfr _internal-exec <state_path> <action_id> [path ...]",
    ...
    # Parse action_id: "group_key.action_key"
    parts = action_id.split(".", 1)
    if len(parts) != 2:
        print(
            f"[fzfr error] Invalid action_id {action_id!r} — expected 'group.action'",
            file=sys.stderr,
        )
        return 1
    group_key, action_key = parts

    # Load state for base_path and query
    ...
    if not group:
        print(f"[fzfr error] No custom action group {group_key!r}", file=sys.stderr)
        return 1
    action = group.get("actions", {}).get(action_key)
    if not action:
        print(
            f"[fzfr error] No action {action_key!r} in group {group_key!r}",
            file=sys.stderr,
        )
        return 1

    # Primary path (first selected, or empty)
    primary = selected_paths[0] if selected_paths else ""
    ...
    if output == "tmux":
        if "tmux" in AVAILABLE_TOOLS:
            ...
            # Fallback: run in current TTY (fzf is suspended by execute-silent,
            # so the terminal is available for the duration of the command).
            r = subprocess.run(cmd, shell=True)
            return r.returncode

    elif output == "preview":
        # Output goes to stdout — fzf will display it in the preview pane
        # when this is called via change-preview+execute.
        r = subprocess.run(cmd, shell=True, stderr=subprocess.STDOUT)
        return r.returncode
        ...
        if r.returncode != 0:
            ...
            if err:
                print(f"[fzfr error] {err}", file=sys.stderr)
        return r.returncode
